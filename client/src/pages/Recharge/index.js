...
import { Form, Input, InputNumber, Select, Tabs, notification } from 'antd';
...
function Recharge({ intl }) {
  ...
  const handleFileChangeIdFront = event => {
    setIsLoading(true);
      ...
      convertFileToBase64(file).then(dataUrl => {
        ...
        if (newData[1]) {
          if (
            ...
            uploadUserIdFront(newData[1], file?.type.replace('image/', ''));
          }
        }
      });
    }
  ...
  function uploadUserIdFront(urlAvatar, type) {
    Upload.uploadImageIdentityCardBefore({
      id: data?.appUserId,
      imageData: urlAvatar,
      imageFormat: type,
    }).then(result => {
  ...
  function uploadUserIdBack(urlAvatar, type) {
    Upload.uploadImageIdentityCardAfter({
      id: data?.appUserId,
      imageData: urlAvatar,
      imageFormat: type,
    }).then(result => {
  ...
  const handleFileChangeIdBack = event => {
    setIsLoading(true);
    const fileObj = event.target.files && event.target.files[0];
    const file = event.target.files[0];
    if (!fileObj) {
      ...
      convertFileToBase64(file).then(dataUrl => {
        ...
        if (newData[1]) {
          if (
            ...
            uploadUserIdBack(newData[1], file?.type.replace('image/', ''));
          }
        }
      });
    }
            ...
            <TabPane
              ...
              {key === 1 && (
                <div>
                  ...
                  <div className="profileUser-BankWallet">
                    <p className="profileUser-BankWallet-title"> </p>

                    {!info.imageBeforeIdentityCard && (
                      <div className="profileUser-avatar">
                        <div className={'profileUser-avatar-content'}>
                          <div
                            ...
                            style={{
                              backgroundImage: `url('${
                                info.imageBeforeIdentityCard ? info.imageBeforeIdentityCard : imgDefault
                              }')`,
                            }}>
                            <div
                        ...
                        <div className={'profileUser-avatar-title'}>CMND mặt trước</div>
                      </div>
                    )}

                    <p className="profileUser-BankWallet-title"> </p>

                    {!info?.imageAfterIdentityCard && (
                      <div className="profileUser-avatar">
                        <div className={'profileUser-avatar-content'}>
                          <div
                            className={'background-image my-profile__avatar mr-px-4'}
                            style={{
                              backgroundImage: `url('${
                                info.imageAfterIdentityCard ? info.imageAfterIdentityCard : imgDefault
                              }')`,
                            }}>
                            <div
                              className={'background-image my-profile__icon'}
                        ...
                        <div className={'profileUser-avatar-title'}>CMND mặt sau</div>
                      </div>
                    )}

                    {info?.imageAfterIdentityCard && info?.imageBeforeIdentityCard && (
                      <p className="profileUser-BankWallet-title"> Đã cập nhật </p>
                    )}
                  </div>
                  <div className="note">
                    <div className="title">Chú ý</div>
