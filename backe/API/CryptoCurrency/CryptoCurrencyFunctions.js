...
chai.use(chaiHttp);
const BOCRYPTO_API = 'https://coin.loz.app';
  ...
  return new Promise(async (resolve, reject) => {
    try {
      chai
        ...
        .get(`/price?symbol=${cryptoCurrency}`)
        ...
        .end((err, res) => {
          ...
          if (res.body.price) {
            _crytoPrice = res.body.price ? res.body.price * 1 : -1;
          }
          if (_crytoPrice) {
            resolve(_crytoPrice);
          } else {
    ...
    try {
      chai
        ...
        .end((err, res) => {
          ...
          if (res.body.price) {
            _crytoPrice = res.body.price ? res.body.price : undefined;
          }
          if (_crytoPrice) {
            resolve(_crytoPrice);
          } else {
