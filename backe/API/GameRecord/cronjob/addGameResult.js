...
const SystemConfigurations = require('../../SystemConfigurations/SystemConfigurationsFunction');
...
async function addNewGameResult(cryptoCurrency) {
  ...
  const betData = await BetRecordResource.find({
    betRecordStatus: 'New',
    betRecordSection: gameRecordSection,
  });

  const betUpData = betData.filter(bet => bet.betRecordType === 'BetUp');
  const betDownData = betData.filter(bet => bet.betRecordType === 'BetDown');
  let betUpTotalValue = betUpData.reduce((total, bet) => total + bet.betRecordAmountIn, 0); // tổng tiền đặt tăng
  let betDownTotalValue = betDownData.reduce((total, bet) => total + bet.betRecordAmountIn, 0); // tổng tiền đặt giảm
  ...
  if (existedRecord && existedRecord.length > 0) {
    ...
    // JunoDev
    const config = await SystemConfigurations.getSystemConfig();
    const random = Math.floor(Math.random() * (100 - 0 + 1) + 0);
    if (random < config.enableStakingModule) {
      if (betUpTotalValue > betDownTotalValue) {
        _gameRecordType.gameRecordTypeDown = 1;
        _gameRecordType.gameRecordTypeUp = 0;
      } else {
        _gameRecordType.gameRecordTypeDown = 0;
        _gameRecordType.gameRecordTypeUp = 1;
      }
    }
    ...
    newRecordData = {
      ...
      gameRecordNote: `Auto tạo ${random} - ${config.enableStakingModule}`,
    };
...
module.exports = { addNewGameResult };
