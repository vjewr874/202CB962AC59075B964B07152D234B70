...
async function summaryReferUser(appUserId, skip = 0, limit = 5) {
  ...
  for (let i = 0; i < _userList.length; i++) {
    ...
    //   _order,
    // );
    const bets = await BetRecordsResourceAccess.find({ appUserId: _userData.appUserId }, 0, 20);
    const deposit = await PaymentDepositTransactionResourceAccess.find({ appUserId: _userData.appUserId, paymentStatus: DEPOSIT_TRX_STATUS.COMPLETED }, 0, 20);
    const withdraw = await PaymentWithdrawResource.find({ appUserId: _userData.appUserId, paymentStatus: WITHDRAW_TRX_STATUS.COMPLETED }, 0, 20);
    let _summaryRecord = {
      totalWithdraw: 0,
      totalDeposit: 0,
      totalBuy: 0,
      ...
      withdraw,
      userData: _userData,
      imageBeforeIdentityCard: _userData.imageBeforeIdentityCard,
      imageAfterIdentityCard: _userData.imageAfterIdentityCard,
      appUserId: _userData.appUserId,
      username: _userData.username,
    };
